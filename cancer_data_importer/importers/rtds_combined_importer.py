...
class RtdsCombinedImporter(BaseImporter):
    ...
    def process_row(self, row):
        try:
            ...
            numeric_fields = ['RTPRESCRIBEDDOSE', 'PRESCRIBEDFRACTIONS', 'RTACTUALDOSE', 'RTACTUALFRACTIONS',
            ...
            values = (
                row['PATIENTID'], row['PRESCRIPTIONID'], row['RADIOTHERAPYEPISODEID'], row['ATTENDID'],
                row['APPTDATE'], row['LINKCODE'], row['RTTREATMENTMODALITY'], row['RTPRESCRIBEDDOSE'], row['PRESCRIBEDFRACTIONS'], row['RTACTUALDOSE'],
                row['RTACTUALFRACTIONS'], row['RTTREATMENTREGION'], row['RTTREATMENTANATOMICALSITE'],
                row['DECISIONTOTREATDATE'], row['EARLIESTCLINAPPROPDATE'], row['RADIOTHERAPYPRIORITY'],
                row['RADIOTHERAPYINTENT'], row['RADIOISOTOPE'], row['RADIOTHERAPYBEAMTYPE'],
                row['RADIOTHERAPYBEAMENERGY'], row['TIMEOFEXPOSURE']
            )
            return sql, values
