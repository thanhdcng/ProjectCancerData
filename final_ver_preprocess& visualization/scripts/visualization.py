...
def visualize_treatment_success_rate(data_path):
    ...
    try:
        data_path = os.path.join(OUTPUT_DIR, "treatment_success_rate.csv")
        ...
        save_visualization(plt, "success_rates", "treatment_success_rate.png")
        plt.close()
...
def visualize_cancer_type_success_rate(data_path):
    ...
    try:
        ...
        save_visualization(plt, "cancer_type", "cancer_type_success_heatmap.png")
        plt.close()
    except Exception as e:
        print(f"❌ Visualization error: {str(e)}")
...
def visualize_kaplan_meier(data_path):
    ...
    try:
        ...
        # Filter data for event_mapped == 'death' and 'survival'
        survival_data = df[df['event_mapped'] != 'unknown']

        plt.figure(figsize=(10, 6))
        ax = plt.gca()

        for regimen in survival_data['standardized_regimen'].unique():
            regimen_data = survival_data[survival_data['standardized_regimen'] == regimen]
        ...
        save_visualization(plt, "survival_analysis", "kaplan_meier_survival_curve.png")
        plt.close()
